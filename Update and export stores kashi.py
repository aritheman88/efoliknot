#!/usr/bin/env python3
"""
Update Store Kashi Price Comparisons and Export to CSV
======================================================

Mirrors Update and export stores promo.py but compares effective prices against
the kashi_items_avg_effective_prices materialized view.

Output:
    kashi/data/store_kashi_comparisons_YYYY-MM-DD.csv  (dated archive)
    kashi/data/store_kashi_comparisons.csv             (for map)

Usage:
    python "Update and export stores kashi.py"  (always prompts interactively)
"""
...
# Chains with promo data — adjust if kashi should cover more/fewer chains
...
OUTPUT_DIR = Path(__file__).parent / 'kashi' / 'data'
...
def build_view_sql(upload_date):
    """
    Generate SQL to create/replace the store_kashi_price_comparisons view.

    Uses kashi_items_avg_effective_prices instead of popular_items_avg_effective_prices.
    Effective price per item: LEAST(regular price, promo price).
    """
    ...
    return f"""
    CREATE OR REPLACE VIEW public.store_kashi_price_comparisons AS
    WITH effective_store_item_prices AS (
        SELECT
            ...
            ki.average_price       AS national_avg_kashi_price,
            CASE
                WHEN ki.average_price > 0 THEN
                    (
                        LEAST(pr.itemprice, COALESCE(ap.discounted_price_per_unit, pr.itemprice))
                        - ki.average_price
                    ) / ki.average_price * 100
                ELSE NULL
        ...
        JOIN kashi_items_avg_effective_prices ki
            ON ki.itemcode = pr.itemcode
    ...
    SELECT
        ...
        COUNT(itemcode)          AS kashi_item_count
...
def update_view(conn, upload_date):
    print(f"[{timestamp()}] Step 1: Updating store_kashi_price_comparisons view...")
    ...
    cursor.execute("""
        ...
        AND table_name = 'store_kashi_price_comparisons'
    """)
...
def export_to_csv(conn, upload_date, output_dir):
    ...
    query = "SELECT * FROM public.store_kashi_price_comparisons ORDER BY store_code"
    ...
    dated_path = output_dir / f"store_kashi_comparisons_{upload_date}.csv"
    df.to_csv(dated_path, index=False, encoding='utf-8-sig')

    map_path = output_dir / "store_kashi_comparisons.csv"
...
def show_statistics(df, upload_date, dated_path, map_path, total_elapsed):
    ...
    print(f"\nKashi price difference statistics:")
    ...
    print(f"\nKashi items per store:")
    print(f"  - Minimum: {int(df['kashi_item_count'].min()):,}")
    print(f"  - Maximum: {int(df['kashi_item_count'].max()):,}")
    print(f"  - Average: {df['kashi_item_count'].mean():.0f}")

    print(f"\nTop 5 cheapest stores (kashi effective prices):")
    for _, row in df.nsmallest(5, 'average_price_diff').iterrows():
        print(f"  {row['store_name']:30} ({row['chainname']:15}, {row['city']:15}): "
              f"{row['average_price_diff']:+6.2f}% | {int(row['kashi_item_count']):,} items")

    print(f"\nTop 5 most expensive stores (kashi effective prices):")
...
def interactive_mode():
    print("\n" + "=" * 70)
    print("Update Store Kashi Price Comparisons and Export")
    print("=" * 70 + "\n")
...
def main():
    ...
    print()
    print("=" * 70)
    print(f"[{timestamp()}] Starting kashi price export for {upload_date}")
    print("=" * 70)
    print()
